...
# check if readiness probe completed in success
if [ ! -f READINESS_PROBE_OK ]; then
  echo "Don't execute Liveness probe because Readiness probe is not completed in success"
  exit 0;
fi
...
if [ ! -z "${wait_for_it}" ]; then
  ...
  if [ "$STATUS" -ne 0 ]; then
    ...
    # check if iep-codec is UP, if not don't send health update
	  ./wait-for-it.sh $iep_codec_host:$iep_codec_port -t 1 -s -- curl -s -m 1 -H "Content-Type:text/plain" --data-binary "$(cat /etc/podinfo/labels > livenessProbe; echo PROBE=LIVENESS_PROBE; echo PROBE_STATUS=FAILED; cat livenessProbe)" -X POST http://$iep_codec_host:$iep_codec_port/health/probe/alerts
    exit 1
  fi
fi
...
if [ "$STATUS" -ge 200 ] && [ "$STATUS" -lt 400 ]; then
  ...
  # check if iep-codec is UP, if not don't send health update
	./wait-for-it.sh $iep_codec_host:$iep_codec_port -t 1 -s -- curl -s -m 1 -H "Content-Type:text/plain" --data-binary "$(cat /etc/podinfo/labels > livenessProbe; echo PROBE=LIVENESS_PROBE; echo PROBE_STATUS=OK; cat livenessProbe)" -X POST http://$iep_codec_host:$iep_codec_port/health/probe/alerts
  exit 0
else
